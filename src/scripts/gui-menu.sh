...
while :; do
    ...
    case "$choice" in
        ...
        "Export Sale Report") export_report sale ;; "Export Detailed Report") export_report detailed ;; "Export Full Report") export_report full ;; "Open Privileged Terminal") zenity --question --title="Privileged local terminal" --text="Changes affect the running ProbeOS session and attached devices.\n\nOpen a root terminal?" && rxvt -title "ProbeOS Privileged Local Terminal" & ;; "Reboot") reboot ;; "Power Off") poweroff ;;
    esac
done
