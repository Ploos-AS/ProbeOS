...
assert_jq "$WORK/full/sale.json" '.profile=="sale" and .system.model=="Fixture Workstation" and (.processor.model|contains("i7-10700")) and .memory.total=="34.4 GB" and (.graphics|length)==2 and (.storage|length)==2 and (.storage[0]|has("health")|not) and (.network|length)==2 and (.firmware.boot_mode=="BIOS" or .firmware.boot_mode=="UEFI") and (.windows.installations|length)==3 and .windows.installations[2].recoverable_key=="not_established" and .windows.firmware_oem_license=="Found" and .batteries[0].health_percent==85 and .hardware_check.status=="Not run"' 'sale report quality without implied diagnostics'
